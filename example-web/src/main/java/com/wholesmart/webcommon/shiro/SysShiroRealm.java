...
public class SysShiroRealm extends AuthorizingRealm{
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken token) throws AuthenticationException {
	       ...
	       User userInfo = userService.selectUserRole(ConvertUtil.toMap("username",(Object)userName));
	       if(userInfo == null){
	    	   throw new UnknownAccountException();// 没找到帐号
	       }
	       if (userInfo.getuLocked()) {
	       ...
	       SimpleAuthenticationInfo authenticationInfo = new SimpleAuthenticationInfo(
				   ...
				   getName() // realm name
			);//此方法将会调用CredentialsMatcher进行密码匹配
		    // 当验证都通过后，把用户信息放在session里
			Session session = SecurityUtils.getSubject().getSession();
			// 用户对象
			ShiroUser shiroUser = new ShiroUser();
			shiroUser.setAccount(userInfo.getUsername());
			shiroUser.setId(userInfo.getId().toString());
			shiroUser.setRealName(userInfo.getRelname());
			session.setAttribute("userSession", shiroUser);
	...
	protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
		 /*
	       ...
	       if (user != null) {
	    	   List<Resource> resourceList = resourceService.findResourcesByUserId(user.getId());
	    	   SimpleAuthorizationInfo info = new SimpleAuthorizationInfo();
	    	   info.addRole(user.getRole().getRoleKey());
